...
def source_to_concentration(
    ...
    """
    First-order mass-balance model for pollutant concentration.

    dC/dt = E/V - (k_vent + k_dep) * C

    emission_rate:
        Effective pollutant source strength in concentration-volume/time
        units after the source-to-concentration scaling.

    volume:
        Effective atmospheric mixing volume.

    ventilation_rate:
        First-order ventilation/removal rate [1/time].

    deposition_rate:
        First-order deposition/removal rate [1/time].

    previous_concentration:
        Concentration at the previous timestep.
    ...
    total_removal = max(
        0.0,
        ventilation_rate + deposition_rate,
    )

    if total_removal == 0.0:
        return (
            previous_concentration
            + (emission_rate / volume) * dt
        )
    ...
    equilibrium_concentration = (
        emission_rate
        / (volume * total_removal)
    )

    return (
        previous_concentration * decay
        + equilibrium_concentration * (1.0 - decay)
    )
...
def _simulate_one(
    ...
    # Fraction of observed background concentration
    # attributed to the traffic source in this prototype.
    traffic_background_fraction = 0.22

    # Baseline traffic-attributable concentration.
    traffic_concentration_base = (
        obs0 * traffic_background_fraction
    )

    # Preserve the traffic source coupling:
    # Q_scenario / Q_base
    traffic_q_base = 180.0

    traffic_ratio = (
        traffic_q_scenario / traffic_q_base
        if traffic_q_base > 0.0
        else 1.0
    )

    # Scenario traffic contribution.
    traffic_concentration_scenario = (
        traffic_concentration_base
        * traffic_ratio
    )

    # Convert the calibrated traffic concentration
    # into an equivalent source term for the mass balance.
    source_rate = (
        traffic_concentration_scenario
        * (vent + k_dep)
        * volume
    ...
    # New mass-balance concentration
    c = source_to_concentration(
        emission_rate=source_rate * emission_scale_val,
        volume=volume,
        ventilation_rate=vent,
        deposition_rate=k_dep,
        previous_concentration=obs0,
    )
    # -----------------------------
    # Gaussian plume
    # -----------------------------
